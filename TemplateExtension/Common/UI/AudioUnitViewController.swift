...
private let log = Logger(subsystem: "com.taylor.audio.TemplateExtension", category: "AudioUnitViewController")

@MainActor
public class AudioUnitViewController: AUViewController, AUAudioUnitFactory {
    var audioUnit: AUAudioUnit?
    
    var hostingController: HostingController<TemplateExtensionMainView>?
	...
	*/
	...
	nonisolated public func createAudioUnit(with componentDescription: AudioComponentDescription) throws -> AUAudioUnit {
		return try DispatchQueue.main.sync {
			...
			audioUnit = try TemplateExtensionAudioUnit(componentDescription: componentDescription, options: [])
			
			guard let audioUnit = self.audioUnit as? TemplateExtensionAudioUnit else {
				log.error("Unable to create TemplateExtensionAudioUnit")
				return audioUnit!
			}
			...
			audioUnit.setupParameterTree(TemplateExtensionParameterSpecs.createAUParameterTree())
    ...
    private func configureSwiftUIView(audioUnit: AUAudioUnit) {
        ...
        let content = TemplateExtensionMainView(parameterTree: observableParameterTree)
