...
private let log = Logger(subsystem: "com.taylor.audio.VXAtomExtension", category: "AudioUnitViewController")

@MainActor
public class AudioUnitViewController: AUViewController, AUAudioUnitFactory {
    var audioUnit: AUAudioUnit?
    
    var hostingController: HostingController<VXAtomExtensionMainView>?
	...
	*/
    ...
    public override func viewDidLoad() {
        ...
        // Register bundled fonts before the SwiftUI view is built
        if let fontURL = Bundle(for: AudioUnitViewController.self)
            .url(forResource: "JackwriteBold", withExtension: "ttf") {
	...
	nonisolated public func createAudioUnit(with componentDescription: AudioComponentDescription) throws -> AUAudioUnit {
		return try DispatchQueue.main.sync {
			...
			audioUnit = try VXAtomExtensionAudioUnit(componentDescription: componentDescription, options: [])
			
			guard let audioUnit = self.audioUnit as? VXAtomExtensionAudioUnit else {
				log.error("Unable to create VXAtomExtensionAudioUnit")
				return audioUnit!
			}
			...
			audioUnit.setupParameterTree(VXAtomExtensionParameterSpecs.createAUParameterTree())
